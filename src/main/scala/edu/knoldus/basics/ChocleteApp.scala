...
case class Chocolate private (max: Int, state: Ref[Int]) {
  ...
  val eat: ZIO[Console, io.Serializable, Unit] = state.modify { oldState =>
    val (newState, action) =
      if (max <= 0)
        (
          oldState,
...
object Chocolate {
  def apply(max: Int): ZIO[Any, Nothing, Chocolate] = Ref.make(0).map(state => new Chocolate(max, state))
}
